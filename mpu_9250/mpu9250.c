...
void mpu9250_read_raw_mag(int16_t mag[3]) {  //Used to get the raw gyro values from the mpu
    uint8_t buffer[6];
...
void calibrate_mag(int16_t magCal[3], int loop)  //Used to calibrate the gyro. The gyro must be still while calibration happens
{
    int16_t temp[3];
    for (int i = 0; i < loop; i++)
    {
